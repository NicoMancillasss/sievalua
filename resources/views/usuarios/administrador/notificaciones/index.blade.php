            ...
            <div class="card-body">
                ...
                @forelse (Auth::user()->unreadNotifications as $notification)
                    <div class="card bg-{{$notification->data['tipo']}} text-white shadow">
                        ...
                        <a href="{{route('administrador.notificacion.show',$notification->id)}}">{{$notification->data['titulo']}}</a>
                         <span style="font-size: 9px">{{$fecha = $notification->created_at->diffForHumans()}} </span>
                        <div class="text-white small">{{$notification->data['mensaje']}}</div>
                        </div>
                    </div>
                @empty
            ...
            <div class="card-body">
                ...
                @forelse (Auth::user()->readNotifications->take(12) as $notification)
                    <div class="card bg-{{$notification->data['tipo']}} text-white shadow">
                        ...
                        <a class="text->withe" href="{{route('administrador.notificacion.show',$notification->id)}}">{{$notification->data['titulo']}}</a>
                         <span style="font-size: 9px">{{$fecha = $notification->created_at->diffForHumans()}} </span>
                        <div class="text-white small">{{$notification->data['mensaje']}}</div>
                        </div>
                    </div>
